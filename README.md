...
### Databricks Labs SDP-META CLI lets you run onboard and deploy in interactive python terminal
...
- Install sdp-meta via Databricks CLI:

```commandline
    databricks labs install sdp-meta
```

### Onboard using sdp-meta CLI:
...
2. Display message: ```sdp-meta pipeline={pipeline_id} created and launched with update_id={pipeline_update_id}, url=https://{databricks workspace url}/#joblist/pipelines/{pipeline_id}```
3. Pipline URL will automatically open in your defaul browser.


### Deploying as a Databricks Asset Bundle (recommended for any non-exploration use)

The interactive `onboard`/`deploy` flow above is great for first-touch exploration, but for anything beyond that — multi-target promotion (`dev` → `prod`), git-tracked pipeline state, or CI/CD — sdp-meta ships a first-class [Databricks Asset Bundle](https://docs.databricks.com/aws/en/dev-tools/bundles/) (DAB) interface that scaffolds a bundle from a packaged template and exposes every knob as a bundle variable.

```commandline
databricks labs install sdp-meta

# Scaffold a bundle from the packaged template (interactive prompts).
databricks labs sdp-meta bundle-init
cd <bundle_name>

# Optional, until sdp-meta is on PyPI: build the wheel and upload to a UC volume,
# auto-pinned into resources/variables.yml as `sdp_meta_dependency`.
...
# sdp-meta-specific sanity checks (placeholder values, layer/topology consistency,
# wheel_source vs sdp_meta_dependency, dataflow_group references) on top of
...
What you get with the bundle path that the interactive CLI doesn't give you:
...
- **`dev` and `prod` targets** out of the box, with development-mode overrides (single-node clusters, no schedules, prefixed table names).
...
- **`bundle-validate` static checks** that catch authoring mistakes the upstream `databricks bundle validate` doesn't (unedited `<your-...>` placeholders, mis-typed `dataflow_group` references, `pipeline_mode` mismatches, etc.).
